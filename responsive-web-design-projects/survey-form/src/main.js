...
function validateForm() {
  const inputs = [...tabs[currTab].querySelectorAll('.form__input, .form__dropdown')];
  ...
  if(!isInputFilled || (radioBtns.length > 0 && !isRadioChecked) || (checkboxes.length > 0 && !isCheckboxChecked)) { 
...
function switchTab(btn) {
  ...
  if(btnIndex === '1') { if(!validateForm()) return; }
  ...
  if(btnIndex === '0' && currTab === 0) { 
    ...
    arrowBtns[0].style.color = '#3e4e56';
  }

  if(btnIndex === '1' && currTab === 2) { 
    btn.target.style.color = 'rgba(62, 78, 86, 0.6)'; 
  } else {
    arrowBtns[1].style.color = '#3e4e56';
  }
  
  formBtns.style.top = `${tabs[currTab].offsetHeight + 59}px`
}
...
window.addEventListener('orientationchange', () => setTimeout(() => formBtns.style.top = `${tabs[currTab].offsetHeight + 63}px`, 200));
